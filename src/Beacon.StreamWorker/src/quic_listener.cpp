...
bool hashes_equal(const TicketHash& left, std::span<const std::byte> right) noexcept {
...
TicketHash hash_stream_ticket(std::span<const std::byte> ticket) {
  ...
  const auto status = BCryptHash(
      ...
      static_cast<ULONG>(ticket.size()), reinterpret_cast<PUCHAR>(result.data()),
...
bool AuthorizedQuicTicketStore::authorize(AuthorizedQuicTicket ticket) {
  if (ticket.client_id.empty() || ticket.session_id.empty() || ticket.plan_revision == 0 ||
      ticket.expires_at_unix_ms == 0) {
    return false;
  }

  std::lock_guard lock{mutex_};
  const auto duplicate = std::ranges::find_if(records_, [&ticket](const Record& record) {
...
QuicTicketConsumeResult AuthorizedQuicTicketStore::consume(
  ...
  std::lock_guard lock{mutex_};
  const auto found = std::ranges::find_if(records_, [&hash](const Record& record) {
    return hashes_equal(record.ticket.hash, hash);
  });
...
class QuicListener::Impl {
 public:
  Impl(std::wstring identity_path, AuthorizedQuicTicketStore& authorized_tickets)
      : identity_path_(std::move(identity_path)), authorized_tickets_(authorized_tickets) {
  ...
  bool configure_listener(std::string_view listen_address, std::uint16_t listen_port) {
    std::lock_guard lock{mutex_};
  ...
  bool open_connection() {
    try {
      ...
      if (QUIC_FAILED(listener_status)) {
        listener_ = nullptr;
        set_failure(QuicListenerFailure::listener_open, status_code(listener_status));
        return false;
      }
      const QUIC_BUFFER alpn{
          static_cast<std::uint32_t>(kBeaconAlpn.size()),
          reinterpret_cast<std::uint8_t*>(const_cast<char*>(kBeaconAlpn.data()))};
      const auto start_status = api_->ListenerStart(listener_, &alpn, 1, &listen_address_);
      if (QUIC_FAILED(start_status)) {
        api_->ListenerClose(listener_);
        listener_ = nullptr;
        set_failure(QuicListenerFailure::listener_start, status_code(start_status));
        return false;
      }

      QUIC_ADDR actual{};
      std::uint32_t actual_size = sizeof(actual);
      if (QUIC_SUCCEEDED(api_->GetParam(
              listener_, QUIC_PARAM_LISTENER_LOCAL_ADDRESS, &actual_size, &actual))) {
  ...
  stream::TransportSendResult send(stream::TransportPacket packet) {
    if (packet.channel != stream::StreamChannel::media || packet.payload.empty()) {
    ...
    auto* context = new DatagramSendContext(std::move(packet.payload), packet.sequence);
 ...
 private:
  struct DatagramSendContext {
    explicit DatagramSendContext(std::vector<std::byte> payload, std::uint64_t value)
  ...
  bool initialize_msquic() {
    ...
    const auto registration_status = api_->RegistrationOpen(
        &registration_config, &registration_);
    if (QUIC_FAILED(registration_status)) {
      set_failure(
          QuicListenerFailure::registration_open, status_code(registration_status));
      return false;
    }
    ...
    if (QUIC_FAILED(configuration_status)) {
      set_failure(
          QuicListenerFailure::configuration_open, status_code(configuration_status));
      return false;
    }
    if (!load_identity()) {
      return false;
    ...
    const auto credential_status = api_->ConfigurationLoadCredential(
        configuration_, &credentials);
    if (QUIC_FAILED(credential_status)) {
      set_failure(QuicListenerFailure::credential_load, status_code(credential_status));
      return false;
    }
  ...
  bool load_identity() {
    ...
    if (!input.read(reinterpret_cast<char*>(pfx.data()), static_cast<std::streamsize>(pfx.size()))) {
    ...
    CRYPT_DATA_BLOB blob{
        static_cast<DWORD>(pfx.size()), pfx.data()};
    ...
    const auto* imported = CertEnumCertificatesInStore(certificate_store_, nullptr);
    if (imported == nullptr) {
      set_failure(QuicListenerFailure::identity_import, GetLastError());
      return false;
    }
    ...
    if (!CertGetCertificateContextProperty(
            certificate_, CERT_KEY_PROV_INFO_PROP_ID, nullptr, &provider_info_size) ||
    ...
    if (!CertGetCertificateContextProperty(
            certificate_, CERT_KEY_PROV_INFO_PROP_ID, provider_info_storage.data(),
            &provider_info_size)) {
      set_failure(QuicListenerFailure::identity_import, GetLastError());
      return false;
    }
  ...
  void delete_imported_key() noexcept {
    ...
    if (NCryptOpenStorageProvider(
            &provider, key_provider_name_.c_str(), 0) != ERROR_SUCCESS) {
      return;
    }
    NCRYPT_KEY_HANDLE key = 0;
    if (NCryptOpenKey(provider, &key, key_container_name_.c_str(), 0, 0) == ERROR_SUCCESS) {
  ...
  void set_failure(QuicListenerFailure failure, std::uint64_t platform_error) noexcept {
    failure_ = failure;
  ...
  [[maybe_unused]] AuthorizedQuicTicketStore& authorized_tickets_;
...
std::uint16_t QuicListener::local_port() const noexcept { return impl_->local_port(); }

QuicListenerFailure QuicListener::failure() const noexcept { return impl_->failure(); }
