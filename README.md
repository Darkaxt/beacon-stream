...
Milestone 101 adds a fakeable Android GameStream RTP decoder consumer that maps server-advertised GameStream codec/container/width/height/fps metadata into the existing MediaCodec surface decoder pipeline. The default APK route still stays RTSP-only until dynamic client port leasing and GameStream-specific RTP depacketization are added.
...
GameStream RTP decoder consumer support reuses the existing `SurfaceEncodedVideoDecoder` boundary when a GameStream/Moonlight descriptor provides codec, container, width, height, and fps metadata. `GameStreamRtpDecoderVideoConsumer` accepts the RTP sample provider produced by the video-session hook and owns decoder stop/release cleanup through `GameStreamRtpVideoConsumer.stop()`. This is still a fakeable boundary: the default APK route remains RTSP-only until live RTP depacketization and route wiring are added.
