...
Android also parses `stream.connection.metadata` and recognizes a decoder-bound `beacon-test` encoded-video contract: `metadata.streamKind=encoded-video`, a `video` endpoint, codec `h264`/`hevc`/`av1`, container `annex-b`/`mp4`, and positive width/height/fps. Valid encoded-video descriptors now enter a fakeable `EncodedVideoDecoder` pipeline and can start a native encoded-video presentation when a decoder is configured. The default APK still reports a truthful no-MediaCodec-decoder-configured diagnostic until the real Android MediaCodec and Surface rendering implementation lands.
