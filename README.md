...
Encoded-video mode advertises `protocol=beacon-test`, no `launchUri`, `video=beacon-test://video/color-bars.h264`, and metadata such as `streamKind=encoded-video`, `codec=h264`, `container=annex-b`, width, height, and fps. This is still a contract/preflight path until real Android MediaCodec transport and decode are implemented.
...
Android also parses `stream.connection.metadata` and recognizes a decoder-bound `beacon-test` encoded-video contract: `metadata.streamKind=encoded-video`, a `video` endpoint, codec `h264`/`hevc`/`av1`, container `annex-b`/`mp4`, and positive width/height/fps. Valid encoded-video descriptors now enter a fakeable `EncodedVideoDecoder` pipeline. The APK wires that pipeline to an Android `SurfaceView` and a `MediaCodec` adapter that can configure, start, stop, and release a decoder for the selected codec and mode. Encoded byte fetching and queueing are still future work, so this is decoder lifecycle readiness rather than visible frame playback.
