...
Milestone 106 accepts H.264 `sprop-parameter-sets` metadata from GameStream descriptors using `h264SpropParameterSets`, `spropParameterSets`, or `sprop-parameter-sets`, decodes SPS/PPS NAL units into Annex B, and prepends them before the first VCL sample. It does not parse RTSP DESCRIBE SDP bodies yet, does not add retransmission, and does not change HEVC or AV1 handling.
