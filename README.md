...
Android native stream handling is routed through protocol-specific clients. The implemented `beacon-test` client renders the diagnostic color-bars presentation, while GameStream/Moonlight descriptors now have a tested RTSP session interface, OPTIONS/DESCRIBE/SETUP/ANNOUNCE/PLAY state machine, structured session id and server-port capture, injectable SDP payload generation, byte-stream transport framing, socket lease creation, APK route wiring, a fakeable video-session hook, and stop-owned lifecycle cleanup. The default APK route remains RTSP-only until UDP/RTP media transport is added behind that hook.
