...
Encoded-video mode advertises `protocol=beacon-test`, no `launchUri`, `video=/streams/beacon-test/color-bars.h264`, and metadata such as `streamKind=encoded-video`, `codec=h264`, `container=annex-b`, width, height, and fps. The server route returns a deterministic short Annex B H.264 diagnostic sequence so emulator testing can exercise multiple encoded frame samples without GameStream/Moonlight.
