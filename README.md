...
Android native stream handling is routed through protocol-specific clients. The implemented `beacon-test` client renders the diagnostic color-bars presentation, while GameStream/Moonlight descriptors now have a tested RTSP session interface, OPTIONS/DESCRIBE/SETUP/ANNOUNCE/PLAY state machine, structured session id and server-port capture, injectable SDP payload generation, byte-stream transport framing, socket lease creation, APK route wiring, a fakeable video-session hook, and stop-owned lifecycle cleanup. Video hook startup/stop exceptions are contained so RTSP session ownership is still released. Behind that hook, `RtpPacket`, `RtpPacketSource`, `GameStreamRtpVideoSampleProvider`, and `GameStreamRtpVideoSessionClient` provide a fakeable RTP packet-to-sample boundary using the RTP 90 kHz video clock for sample timestamps. The default APK route remains RTSP-only until real UDP socket intake, GameStream-specific depacketization, and MediaCodec consumer wiring are added behind that hook.
