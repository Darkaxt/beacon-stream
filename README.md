...
> **Architecture recovery is active.** The authoritative design is
> [`docs/superpowers/specs/2026-06-03-personal-streaming-orchestrator-design.md`](docs/superpowers/specs/2026-06-03-personal-streaming-orchestrator-design.md).
> Beacon now targets one Beacon-owned StreamWorker and one thin Android StreamCore path. Apollo, Sunshine,
> GameStream, Moonlight, external-wrapper, runtime-descriptor, and parallel Android compatibility paths are
> removal targets, not product requirements. The milestone descriptions below record the implementation currently
> present in `main`; they do not override the recovery specification.

Milestone 0/1 covers the control plane, fake backends, planner, profile ownership, phone-free testing, and source-boundary documentation. Milestone 2 adds the real Windows SudoVDA/DisplayConfig lifecycle backend and manual no-phone display probe. Milestone 3 adds the normalized game library model, Steam/Heroic/Hydra/manual providers, SteamGridDB/fallback artwork providers, server/client-lab game selection, and a read-only local game probe. Milestone 4 adds a WPF cockpit for local server administration. Milestone 5 adds the streaming backend boundary, fake no-phone stream lifecycle, and external-process adapter boundary for future Sunshine-compatible integration. Milestone 6 adds a thin Android control-plane APK shell. Milestone 7 adds the server-owned game launch and session ownership cleanup boundary. Milestone 8 adds the Windows process/window activity inspector. Milestone 9 adds explicit fake-vs-Windows server host composition. Milestone 10 adds explicit streaming backend selection and preflight before display/app side effects. Milestone 11 adds manual recovery actions for stranded windows/processes. Milestone 12 adds persistent client profiles and an explicit pairing boundary for new clients. Milestone 13 adds cockpit profile editing. Milestone 14 adds selected-client recovery/admin actions. Milestone 15 adds telemetry-driven initial planning. Milestone 16 brings Android preflight payloads up to the server planning contract. Milestone 17 adds typed stream connection descriptors. Milestone 18 adds an external-wrapper readiness manifest for capability preflight and descriptor discovery. Milestone 26 makes Android delegate `stream.connection.launchUri` through Android's normal URI launcher after successful launch. Milestone 27 makes disconnect/quit report stream backend stop failures instead of continuing into misleading recovery state. Milestone 28 adds an explicit reset-topology recovery action. Milestone 29 posts Android stream URI handoff onto the Activity UI thread before Android intent launch. Milestone 30 makes the CLI fake endpoint exercise launch and reconnect in the no-phone script. Milestone 31 includes display mode and HDR/SDR rationale in the plan display reason and Client Lab output. Milestone 32 lets Android fetch and show the server-owned game catalog. Milestone 34 lets Android use a loaded catalog entry as the plan/launch target while keeping raw game id fallback. Milestone 35 gives APK-local interaction settings a SharedPreferences-backed persistence boundary without adding server display policy. Milestone 36 exposes read-only display driver/topology health in the admin snapshot and Cockpit diagnostics. Milestone 37 exposes read-only streaming backend health, wrapper manifest capability, and active-stream counts. Milestone 38 reconciles external wrapper process liveness on demand so exited wrappers stop appearing as running streams. Milestone 39 captures bounded stdout/stderr tails from external wrappers for failure diagnostics. Milestone 40 documents and machine-checks the external wrapper manifest contract. Milestone 41 adds a typed client input forwarding contract and no-phone simulator coverage. Milestone 42 publishes input forwarding diagnostics through the operational journal. Milestone 43 adds a source-audited Windows host input sink for display-targeted pointer `move`, `down`, `up`, and `tap` events. Milestone 44 exposes read-only input backend health in the admin snapshot and Cockpit diagnostics. Milestone 45 adds a JVM-tested Android touch-to-pointer mapper and simple single-pointer touch surface. Milestone 46 makes the no-phone simulators send a deterministic down/move/up pointer gesture instead of only a fixed center tap. Milestone 47 adds Windows host keyboard `down`, `up`, and `press` input support with health reporting. Milestone 48 makes Client Lab and the CLI fake endpoint exercise deterministic keyboard input without a phone. Milestone 49 adds Android-side Escape keyboard input forwarding through the same input route. Milestone 50 wires APK-local theme, wake-lock, and decoder debug overlay controls without adding server display policy. Milestone 51 adds Android multi-pointer touch batches through the existing pointer route. Milestone 52 adds external-wrapper runtime session descriptors so a started wrapper can publish actual stream launch URI/endpoints without phone testing. Milestone 53 adds a no-phone streaming wrapper probe executable that writes the same runtime descriptor contract and stays alive until Beacon stops it. Milestone 54 adds a server API smoke test that launches that probe through the real Windows external-process runner while keeping display and game side effects fake. Milestone 55 lets the CLI fake endpoint optionally assert that a launched session exposes a usable `stream.connection` descriptor before it sends input. Milestone 56 adds a stream-handoff-only fake endpoint script mode and an in-memory server integration test that launches the real wrapper probe, validates immediate GameStream-style connection metadata, observes the runtime descriptor, and verifies cleanup. Milestone 57 launches external wrapper processes from the wrapper executable directory so relative config/log assumptions do not depend on Beacon's own working directory. Milestone 62 lets the streaming probe supervise a configured child process before publishing runtime descriptor evidence, which gives Beacon a no-phone harness for wrapper process ownership before real Sunshine-compatible integration. Milestone 63 adds server-owned configuration for that child process and preflights a missing configured child before display or app side effects. Milestone 64 adds a server-level no-phone smoke that launches the probe with a real child process and verifies child exit reconciliation through the server stream API. Milestone 65 exposes structured wrapper-child readiness in admin and Cockpit streaming health. Milestone 66 rejects wrapper child arguments without a child executable. Milestone 76 adds a server-owned external wrapper argument template with preflight validation. Milestone 77 adds an Android in-app native stream boundary for emulator-testable `beacon-test` descriptors. Milestone 78 adds a selectable `beacon-test` streaming backend and an Android in-app color-bars test-pattern surface. Milestone 79 makes Android validate endpoint-only GameStream/Moonlight maps and report incomplete handoff versus complete-but-decoder-not-implemented diagnostics. Milestone 80 replaces Android's hardcoded codec facts with a device-derived decoder capability probe. Milestone 81 enriches Android telemetry reports with device-derived battery, thermal, and Wi-Fi transport facts. Milestone 82 splits Android native stream handling into protocol-specific clients behind a router. Milestone 84 lets the server-side `beacon-test` backend emit the Android-readable encoded-video descriptor under explicit test configuration. Milestone 85 adds a fakeable Android encoded-video decoder pipeline boundary so valid descriptors can start decoder-backed native streams when a decoder is configured. Milestone 86 wires Android encoded-video streams to a MediaCodec/Surface configure/start/release lifecycle. Milestone 87 adds server-served diagnostic H.264 bytes and Android fetch/queue plumbing for that encoded-video path. Milestone 88 turns that path into a short Annex B sample sequence with Android access-unit splitting and frame timestamps. Milestone 89 adds a server-framed Annex B sample transport with explicit sample timestamps that Android prefers when advertised. Milestone 90 adds a fakeable Android GameStream RTSP endpoint/session boundary and deterministic RTSP message codec. Milestone 91 adds a fakeable RTSP OPTIONS/DESCRIBE handshake client behind that boundary. Milestone 92 extends that fakeable RTSP state machine through audio/video/control SETUP negotiation with session id capture and server RTP port parsing. Milestone 93 extends the same fakeable RTSP state machine through injectable-SDP ANNOUNCE and PLAY sequencing. Milestone 94 adds a tested RTSP byte-stream transport boundary with deterministic close and transport diagnostics. Milestone 95 adds injectable Android RTSP socket leases, session-retained transport lifecycle, and stop cleanup. Milestone 96 wires the APK GameStream route to that RTSP socket session client and retains one Activity model/native-stream session across actions so Stop can release it. Milestone 97 adds structured RTSP session info and an optional fakeable GameStream video-session hook. Milestone 98 adds a fakeable RTP packet parser/source/sample-provider boundary behind that hook. Milestone 99 makes RTSP SETUP advertise explicit per-stream client RTP ports and retain them in session info for future UDP intake. Milestone 100 adds a real/fakeable UDP datagram-to-RTP packet source bound to the negotiated video client port. Milestone 105 adds bounded RTP sequence reordering before sample/depacketization, without retransmission or timeout-based loss recovery. Milestone 106 adds descriptor-metadata H.264 `sprop-parameter-sets` parsing and SPS/PPS injection before the first video-coded sample. Milestone 107 preserves RTSP DESCRIBE response bodies, extracts H.264 `sprop-parameter-sets` from the SDP video section, and uses that value as an RTP SPS/PPS injection fallback. Milestone 108 groups H.264 RTP packets with the same RTP timestamp into one decoder access-unit sample and flushes on marker bit, timestamp change, or source end. Milestone 109 cleans active GameStream video/RTSP state before replacement starts and adds JVM loopback coverage from RTSP setup into H.264 RTP sample consumption. Milestone 110 packages a pinned `moonlight-common-c` transport core, Milestone 111 adds the strict server-to-APK native session contract, and Milestone 112 routes that contract through `LiStartConnection` into the APK's Surface-backed `MediaCodec` video path. Server-owned pairing and `/launch` provisioning, native audio/input callbacks, controller/touch protocol support, and HDR display-mode switching remain later work.

Milestone 101 adds a fakeable Android GameStream RTP decoder consumer that maps server-advertised GameStream codec/container/width/height/fps metadata into the existing MediaCodec surface decoder pipeline.

Milestone 102 wires the default Android GameStream route into the UDP RTP sample-provider and MediaCodec decoder consumer only when the descriptor advertises RTP decoder metadata. Descriptors without that metadata preserve the RTSP-only route, while partial metadata produces a clear video-start failure instead of silently pretending media is active.

Milestone 103 adds H.264 RTP depacketization for single NAL, STAP-A, and FU-A payloads into Annex B samples before MediaCodec. Later milestones add RTP port leasing, sequence reordering, descriptor-metadata parameter-set injection, and RTSP DESCRIBE SDP parameter-set extraction; HEVC, AV1, and jitter timing remain future work.

Milestone 104 leases Android RTP UDP sockets dynamically before RTSP SETUP, advertises the actual leased client ports, and reuses the leased video socket for RTP intake. Later milestones add RTP sequence reordering, descriptor-metadata H.264 parameter-set injection, and RTSP DESCRIBE SDP parameter-set extraction; retransmission, audio/control RTP processing, and HEVC/AV1 depacketization remain future work.

Milestone 105 adds a bounded RTP sequence reordering source before GameStream RTP sample/depacketization. It handles small UDP packet reordering, duplicate/late packet drops, 16-bit RTP sequence wraparound, and count-bounded missing-packet recovery without sleeps, socket timeouts, cancellation timeouts, or retransmission.

Milestone 106 accepts H.264 `sprop-parameter-sets` metadata from GameStream descriptors using `h264SpropParameterSets`, `spropParameterSets`, or `sprop-parameter-sets`, decodes SPS/PPS NAL units into Annex B, and prepends them before the first VCL sample. It does not add retransmission and does not change HEVC or AV1 handling.

Milestone 107 preserves RTSP DESCRIBE response bodies, extracts H.264 `sprop-parameter-sets` from the SDP video section, carries that value in `GameStreamRtspSessionInfo`, and uses it as a fallback for RTP SPS/PPS injection when descriptor metadata does not already provide parameter sets. It does not add HEVC/AV1 depacketization, jitter timing, retransmission, audio/control RTP processing, input protocol support, controller, or native touch/gesture protocol support.

Milestone 108 groups H.264 RTP packets that share an RTP timestamp into one Annex B access-unit sample, flushing on RTP marker bit, timestamp change, or source end. It does not add jitter timing, retransmission, HEVC/AV1 depacketization, audio/control RTP processing, input protocol support, controller, or native touch/gesture protocol support.
...
Milestone 111 defines that complete server-owned native session descriptor. External wrappers can publish every `SERVER_INFORMATION` and `STREAM_CONFIGURATION` input, including per-session remote-input AES material, through the runtime descriptor. Beacon validates and retains that object outside public stream/admin state, returns it only on client launch and stream-status routes, and removes it on stop or process exit. Android strictly maps the wire names to the pinned `moonlight-common-c` constants without changing server policy. The checked contract lives in `docs/external-streaming-wrapper-manifest.md` and `docs/examples/external-streaming-runtime-session.example.json`; calling `LiStartConnection` remains the next host-provisioning/media milestone.
...
The same backend can emit the Android-readable encoded-video descriptor instead of the color-bars pattern descriptor:
...
Encoded-video mode advertises `protocol=beacon-test`, no `launchUri`, `video=/streams/beacon-test/color-bars.h264`, `samples=/streams/beacon-test/color-bars.beacon-annexb`, and metadata such as `streamKind=encoded-video`, `codec=h264`, `container=annex-b`, `sampleTransport=beacon-annexb-samples`, width, height, and fps. The raw H.264 route returns a deterministic short Annex B diagnostic sequence. The sample route wraps that same sequence in a Beacon binary envelope with magic `BEACONANNEXB1\n` and little-endian `presentationTimeUs`/sample-length records, so emulator testing can exercise multiple encoded frame samples without deriving timing from a plain file. Full GameStream/Moonlight transport remains future work.
...
`Beacon.Android` is a thin Java APK shell for the client control plane. It can identify the device, patch only APK-allowed client profile fields, fetch and show the server-owned game catalog, report device-derived decoder capability facts and telemetry facts, request/launch a server plan, delegate fallback `stream.connection.launchUri` values through Android `ACTION_VIEW`, start diagnostic `beacon-test` streams, and route a validated owning-client `nativeSession` through the packaged Moonlight core before considering that fallback URI. Native connection/video callbacks feed a bounded blocking Annex-B sample queue consumed by the existing Surface-backed `MediaCodec` adapter, with native timestamps and deterministic stop cleanup. Descriptors without `nativeSession` retain the existing endpoint-only Java RTSP/RTP fallback. The APK also supports stop/disconnect/quit, pointer and keyboard forwarding, APK-local interaction settings, and owning-client emergency restore. Display behavior policy remains in Beacon Server. Live Apollo streaming is not yet claimed because server-owned pairing and `/launch` provisioning are still missing; native audio, input, controller/touch, and HDR display-mode callbacks are also later work.
...
GameStream RTP decoder route support reuses the existing `SurfaceEncodedVideoDecoder` boundary when a GameStream/Moonlight descriptor provides codec, container, width, height, and fps metadata. `GameStreamRtpDecoderVideoConsumer` accepts the RTP sample provider produced by the video-session hook, returns an `encoded-video` presentation so the Activity reveals the shared `SurfaceView`, and owns decoder stop/release cleanup through `GameStreamRtpVideoConsumer.stop()`. Descriptors without RTP decoder metadata keep the RTSP-only route; partial metadata enters the video path and fails with the video diagnostic. H.264 RTP descriptors now use an H.264 payload reader that converts single NAL, STAP-A, and FU-A packets into Annex B bytes, groups same-timestamp RTP packets into access-unit samples flushed by marker bit, timestamp change, or source end before MediaCodec, wraps RTP packet intake in bounded sequence reordering before provider selection, and can prepend descriptor-provided or RTSP DESCRIBE SDP-provided SPS/PPS parameter sets before the first VCL sample. HEVC, AV1, jitter timing, retransmission, and audio/control RTP processing remain later work.
...
For endpoint-only GameStream/Moonlight descriptors, Android validates the server-owned endpoint map before entering the native stream boundary. Required roles are `rtsp`, `video`, `control`, and `audio`. Missing roles produce an incomplete-handoff diagnostic. Complete maps must expose an `rtsp://` endpoint with host and port; invalid RTSP endpoints produce explicit diagnostics. Valid maps route through a fakeable `GameStreamRtspSessionClient`, small RTSP request/response codec, and injectable `GameStreamRtspHandshakeClient` that can run OPTIONS, DESCRIBE, audio/video/control SETUP, ANNOUNCE, and PLAY over a fake transport. SETUP captures the RTSP session id and parses server RTP ports from `Transport: server_port=...`; successful handshakes now expose that data plus SDP-derived H.264 `sprop-parameter-sets` as `GameStreamRtspSessionInfo` instead of requiring status-string parsing. ANNOUNCE uses an injectable SDP payload provider so the diagnostic boundary can later be replaced with real GameStream session parameters. `RtspByteStreamTransport` can write serialized requests, read framed RTSP responses, preserve `Content-Length` bodies for SDP metadata extraction, close owned streams, and convert I/O failures into handshake diagnostics. `GameStreamRtspTransportSessionClient` can open an injectable `RtspTransportLease`, retain it after a successful handshake, close failed or replaced leases, and release it from native stream `stop()`. `RtspSocketTransportLeaseFactory` can build that lease from an injectable `RtspSocketConnector`, including the real `JavaRtspSocketConnector`, without connect timeouts, socket timeouts, sleeps, or retries. The APK Activity composes this socket-backed RTSP session client through `AndroidNativeStreamClientFactory`, and `BeaconViewModelSession` retains the model/native-stream client across button actions so Stop can release the active RTSP lease. Session cleanup detaches the retained model before releasing native stream state, so an unchecked native cleanup failure cannot pin Activity actions to stale stream state. HEVC/AV1 depacketization, jitter timing, retransmission, and full Moonlight/GameStream media decode remain future work.

Android native stream handling is routed through protocol-specific clients. The implemented `beacon-test` client renders the diagnostic color-bars presentation, while GameStream/Moonlight descriptors now have a tested RTSP session interface, OPTIONS/DESCRIBE/SETUP/ANNOUNCE/PLAY state machine, structured session id plus server/client RTP port capture, SDP-derived H.264 parameter-set capture, injectable SDP payload generation, byte-stream transport framing, socket lease creation, APK route wiring, a fakeable video-session hook, and stop-owned lifecycle cleanup. Replacement starts first release any active GameStream video and RTSP state so failed replacement attempts cannot leave stale video state running. Behind that hook, `RtpPacket`, `RtpPacketSource`, `RtpReorderingPacketSource`, `H264ParameterSets`, `GameStreamRtpVideoSampleProvider`, `H264RtpSampleProvider`, and `GameStreamRtpVideoSessionClient` provide a fakeable RTP packet-to-sample boundary using the RTP 90 kHz video clock for sample timestamps, including H.264 single NAL, STAP-A, and FU-A conversion to Annex B access-unit samples when codec metadata is `h264`, descriptor-metadata or RTSP DESCRIBE SDP SPS/PPS injection before the first VCL sample, and bounded RTP sequence reordering before sample/depacketization. `GameStreamRtpPortLease`, `RtpDatagramSocket`, `RtpDatagramPacketSource`, `JavaRtpDatagramSocketFactory`, and `GameStreamUdpRtpPacketSourceFactory` add a real/fakeable UDP datagram intake boundary that dynamically leases RTP sockets before RTSP SETUP, advertises their actual client ports, and transfers the leased video socket into packet intake. The default APK route now composes that hook with `GameStreamRtpDecoderVideoConsumer` only when decoder metadata is advertised, and JVM loopback coverage drives that route from RTSP setup into H.264 RTP sample consumption without a phone. HEVC/AV1 depacketization, jitter timing, retransmission, audio/control RTP processing, input protocol support, controller, and native touch/gesture protocol support remain future work.

Android also parses `stream.connection.metadata` and recognizes a decoder-bound `beacon-test` encoded-video contract: `metadata.streamKind=encoded-video`, a `video` endpoint, codec `h264`/`hevc`/`av1`, container `annex-b`/`mp4`, and positive width/height/fps. Valid encoded-video descriptors now enter a fakeable `EncodedVideoDecoder` pipeline. The APK wires that pipeline to an Android `SurfaceView`, resolves server-relative encoded-video paths against the configured Beacon server URL, and queues samples into `MediaCodec` through callback-driven input/output handling. When metadata advertises `sampleTransport=beacon-annexb-samples`, Android fetches the `samples` endpoint and uses the server-framed presentation timestamps. Older descriptors keep the raw H.264 fallback path that splits Annex B access units and assigns monotonic timestamps from the advertised FPS. This is still a deterministic diagnostic byte path; full GameStream/Moonlight transport remains future work.
...
- `docs/superpowers/plans/2026-07-09-beacon-stream-milestone-84-beacon-test-encoded-video-mode.md`
- `docs/superpowers/plans/2026-07-09-beacon-stream-milestone-85-android-encoded-video-pipeline.md`
- `docs/superpowers/plans/2026-07-09-beacon-stream-milestone-86-android-mediacodec-surface-lifecycle.md`
- `docs/superpowers/plans/2026-07-09-beacon-stream-milestone-87-encoded-video-byte-path.md`
- `docs/superpowers/plans/2026-07-09-beacon-stream-milestone-88-annexb-sample-sequence.md`
- `docs/superpowers/plans/2026-07-09-beacon-stream-milestone-89-framed-sample-transport.md`
- `docs/superpowers/plans/2026-07-09-beacon-stream-milestone-90-gamestream-rtsp-boundary.md`
- `docs/superpowers/plans/2026-07-09-beacon-stream-milestone-91-rtsp-handshake-client.md`
- `docs/superpowers/plans/2026-07-09-beacon-stream-milestone-92-rtsp-setup-negotiation.md`
