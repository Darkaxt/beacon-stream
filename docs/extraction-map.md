...
| --- | --- | --- | --- |
| Sunshine | Streaming protocol, capture, encode, audio, input reference; Milestone 10/17 external process boundary and Beacon-owned connection metadata only | No | `src/Beacon.Platform.Windows/Streaming/ExternalProcessStreamingBackend.cs` passes Beacon plans and optional connection descriptors to a configured wrapper without copying Sunshine source |
