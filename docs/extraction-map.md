...
| --- | --- | --- | --- |
| Sunshine | Streaming protocol, capture, encode, audio, input reference; Milestone 10/17/18/40 external process boundary, Beacon-owned connection metadata, wrapper readiness manifest, Milestone 41 input contract, and Milestone 43 Windows input targeting principle only | No | `src/Beacon.Platform.Windows/Streaming/ExternalProcessStreamingBackend.cs` passes Beacon plans, optional connection descriptors, and a Beacon-owned manifest path to a configured wrapper without copying Sunshine source; `src/Beacon.Core/Input/ClientInput.cs` defines an original input forwarding contract without copying Sunshine source; `src/Beacon.Platform.Windows/Input/WindowsClientInputSink.cs` resolves Beacon display topology before dispatching pointer input without copying Sunshine source; `docs/external-streaming-wrapper-manifest.md` and `docs/source-audits/2026-07-08-windows-input-sink-upstream-audit.md` document adapter/input boundaries without copying Sunshine source |
| Moonlight/Artemis Android clients | Android client behavior reference for local-vs-server ownership and client-local input settings | No | `src/Beacon.Android` is original Java code for Beacon's control plane only; no client streaming source is copied; `docs/source-audits/2026-07-08-windows-input-sink-upstream-audit.md` records the client-local input settings comparison |
